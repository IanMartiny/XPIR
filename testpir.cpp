...
bool run(DBHandler *db, uint64_t chosen_element, PIRParameters params){
  ...
  ******************************************************************************/
  ...
  // using pushQuery
  char* query_element;
  uint64_t query_size = q_generator.getQueryElementBytesize();
  std::cout << "Query elements are " << query_size << " bytes" << std::endl;
  uint64_t query_count = 0;
  while (q_generator.popQuery(&query_element))
  {
    query_count++;
    //std:: cout << "    query " << query_count << std::endl;
    r_generator->pushQuery(query_element);
  }
  std::cout << "There were " << query_count << " elements total, for " << query_count*query_size << " total bytes" << std::endl;
  ...
  ******************************************************************************/
  ...
  char* reply_element;
  uint64_t reply_size = r_generator->getReplyElementBytesize();
  std::cout << "Reply elements are " << reply_size << " bytes" << std::endl;
  ...
  std::cout << "and there were " << reply_count << " of them (" << reply_count*reply_size << " bytes)" << std::endl;
...
/******************************************************************************
  ...
  ******************************************************************************/
  ...
  // Test with recursion 3 and huge file/db lots of files
  std::cout << "======================================================================" << std::endl;
  std::cout << "Test 8/8: database_size = 1ULL<<30; nb_files = 1024*1024;" << std::endl;
  std::cout << "params.alpha = 1; params.d = 3; crypto_params = LWE:80:2048:120;" << std::endl; 
  std::cout << "======================================================================" << std::endl;
  database_size = 1ULL<<23; nb_files = 1024*64; maxFileBytesize = database_size/nb_files;
  DBGenerator db8(nb_files, maxFileBytesize, /*bool silent*/ false); 
  chosen_element = 22573;
  params.alpha = 1; params.d = 1; params.n[0] = 1024*64; //params.n[0] = 16; params.n[1] = 16; params.n[2] = 16; params.n[3] = 16;
  params.crypto_params = "LWE:80:2048:120";
  //params.crypto_params = "Paillier:128:3072:6144";
  //params.crypto_params = "LWE:91:2048:120";
